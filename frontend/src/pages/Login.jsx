...
function Login() {
  ...
  const { mutate: loginMutation, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(data.message || 'Welcome back to Bookify')
      ...
      <section className="mx-auto grid max-w-[960px] overflow-hidden rounded-[28px] border border-black/8 bg-white shadow-[0_24px_65px_rgba(30,24,18,0.1)] lg:grid-cols-[0.9fr_1.1fr]">
        <div className="relative hidden overflow-hidden bg-bookify-primary-dark p-10 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="relative py-16">
            ...
            <h1 className="mt-6 font-serif text-4xl font-bold leading-tight tracking-[-0.04em]">
              Welcome back to your university bookshop.
            </h1>
            <p className="mt-4 text-sm leading-6 text-white/70">
              Sign in to manage your cart, complete orders, and access your Bookify account.
            </p>
          </div>
        ...
        <div className="px-6 py-10 sm:px-10 sm:py-12 lg:px-14">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <label className="block text-sm font-semibold text-bookify-ink">
              <span className="flex items-center justify-between gap-4">
                Password
                <a href="#forgot-password" className="text-xs font-semibold text-bookify-primary hover:text-bookify-primary-dark">
                  Forgot password?
                </a>
              </span>
              <span className="relative mt-2 block">
            ...
            <label className="flex cursor-pointer items-center gap-2.5 text-sm text-bookify-muted">
              <input
                type="checkbox"
                name="remember"
                className="checkbox checkbox-sm border-black/20 [--chkbg:var(--color-bookify-primary)] [--chkfg:white]"
              />
              Remember me
            </label>
            ...
            <button
              type="submit"
              disabled={isPending}
              className="h-12 w-full rounded-xl bg-bookify-primary text-sm font-bold text-white shadow-[0_10px_25px_rgba(8,120,63,0.17)] transition hover:bg-bookify-primary-dark disabled:cursor-wait disabled:opacity-70"
            >
          ...
          <p className="mt-6 text-center text-sm text-bookify-muted">
            ...
            <Link to="/register" className="font-bold text-bookify-primary hover:text-bookify-primary-dark">
              Create account
            </Link>
          </p>
        </div>
